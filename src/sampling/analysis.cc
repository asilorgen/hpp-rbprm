...
namespace
{
    ...
    rbprm::RbPrmLimbPtr_t getLimbFromStartRank(size_t startRank,rbprm::RbPrmFullBodyPtr_t fullBody  ){
      ...
      hppDout(notice,"ref config rank: "<<cit->second->limb_->rankInVelocity()<<" ; "<<cit->second->effector_->rankInVelocity());  //AOrgen
    ...
    //assign weight for each sample with compared reference config and axis!!
    double referenceConfiguration(rbprm::RbPrmFullBodyPtr_t fullBody , const SampleDB& /*sampleDB*/, const sampling::Sample& sample){
      ...
      hppDout(notice, "Limb config dondu mu?" << device->currentConfiguration().transpose());  //AOrgen
      ...
      hppDout(notice,"weight(size = number of joint in each limb)=" << weight); //AOrgen
      ...
      // by looking at the jacobian of each joint we can check around which axis the joint is.
      ...
      hppDout(notice,"weight array, after process"<<model::displayConfig(weight));    //AOrgen - comment it out!!
      hpp::model::difference (device, conf, fullBody->referenceConfig(), diff);
      hppDout(notice,"Reference config in analysis : "<<model::displayConfig(fullBody->referenceConfig())); //TODO: correct this!!
      ...
      if(sqrt(distance)>=100){
        hppDout(error,"WARNING : max distance to config not big enough"); //AOrgen this config is not really good choise!! TODO: correct!!
      }
    ...
    : device_(device)
{
    ...
    evaluate_.insert(std::make_pair("TrotReference", boost::bind(&trotReference, boost::ref(device_), _1, _2)));
}
